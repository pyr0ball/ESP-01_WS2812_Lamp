...
void ssidResetConfig () {
    ...
    // Erase EEPROM to avoid stack overflow
...
void setup ( void ) {
    ...
    Serial.println("Initializing NeoLamp");
    Serial.println("Connecting to WiFi...");
    // Fallback to ConfigAP if stored wifi is unable to connect
  ...
  EEPROM.begin(5);                           // set up EEPROM storage space for config values
	...
	if ( mdns.begin ( "NeoLamp", WiFi.localIP() ) ) {
  ...
  server.on ( "/settings_page", handleSettings);
  ...
  server.on ( "/ssidReset", ssidResetConfig);
  server.on ( "/restartWifi", restartWifi);
